<template>
    ...
    >
        <div class="pointer-events-auto w-full relative z-50">
            ...
            >
                <div class="flex flex-1 items-center gap-2 overflow-hidden font-mono text-xs font-semibold">
                    ...
                    <span class="whitespace-nowrap tracking-wider text-gray-200">
                        <span v-if="splitTime.date" class="hidden sm:inline mr-1">{{ splitTime.date }}</span>
        ...
        >
            ...
            >
                <div class="flex items-center justify-between mb-3 border-b border-white/10 pb-2">
                    ...
                    <button @click="showSettings = false" class="text-gray-500 hover:text-white">
                    ...
                    <div class="space-y-1.5 flex-[1.5]">
                        ...
                        <div class="relative">
                            ...
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-400">
                                <UIcon name="i-heroicons-chevron-up-down-20-solid" class="h-3 w-3" />
                            </div>
                        </div>
                    </div>
        ...
        <div class="pointer-events-auto w-full transition-all duration-300">
            <div v-if="status === 'error'" class="flex items-center justify-center gap-2 rounded-lg border border-red-500/50 bg-red-950/80 p-2.5 text-red-100 backdrop-blur shadow-lg">
            ...
            <div v-else-if="!eewData" class="flex items-center justify-center rounded-lg border border-gray-800 bg-gray-900/80 p-2.5 text-gray-400 backdrop-blur shadow-lg">
            ...
            <div v-else-if="isEmptyResponse" class="hidden rounded-lg border border-gray-800 bg-black/40 p-2 text-center text-xs text-gray-500 backdrop-blur-sm shadow-sm">
                <span>특이사항 없음</span>
            </div>
            <div v-else class="relative overflow-hidden rounded-xl border shadow-2xl backdrop-blur-md transition-all duration-300" :class="[isCancel ? 'border-gray-600 bg-gray-800/90' : 'border-red-500/30 bg-gray-900/90 shadow-red-900/20']">
                <div class="absolute inset-y-0 left-0 w-1" :class="isCancel ? 'bg-gray-500' : 'bg-red-600'"></div>
                <div class="flex items-center gap-3 p-3 pl-4">
                    <div class="flex h-12 w-12 flex-shrink-0 flex-col items-center justify-center rounded-lg shadow-sm" :class="[isCancel ? 'bg-gray-600 text-gray-300' : 'bg-gradient-to-br from-yellow-400 to-orange-500 text-white']">
                        <span class="text-[0.55rem] font-bold opacity-90 leading-none mb-0.5">진도</span>
                        <span class="font-mono text-2xl font-black leading-none tracking-tighter shadow-black drop-shadow-sm">{{ displayIntensity }}</span>
                    </div>
                    <div class="flex min-w-0 flex-1 flex-col justify-center">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="flex-shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold text-white shadow-sm" :class="isCancel ? 'bg-gray-500' : 'bg-red-600'">{{ isCancel ? "취소" : "속보" }}<span class="opacity-75">| {{ isFinal ? "최종" : `#${eewData.report_num}` }}</span></span>
                            <span class="truncate text-lg font-bold leading-tight text-white drop-shadow-md">{{ eewData.region_name }}</span>
                        </div>
                        <div class="flex items-center gap-3 text-xs font-medium text-gray-300">
                            <span class="font-mono text-gray-400">{{ formatOriginTime(eewData.origin_time).split(" ")[1] }}</span>
